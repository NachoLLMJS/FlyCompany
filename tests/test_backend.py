...
class BackendTests(unittest.TestCase):
    ...
    def test_research_persists_only_real_extracted_sources(self):
        ...
        rss = b'<rss><channel><item><title>BNB test fixture</title><link>https://www.coindesk.com/test</link><description>Evidence fixture</description></item></channel></rss>'
        def fetch(url):
            return rss if url.rstrip('/').endswith('rss') else b'<html><title>Flap docs fixture</title><body>Documented launch workflow and human review.</body></html>'
        with patch.object(backend, 'fetch_source', side_effect=fetch):
            result = self.app.research()
        self.assertEqual(len(result['findings']), 2)
        self.assertEqual(len(backend.App(self.path, backend.Provider({})).state()['findings']), 2)
...
class HTTPTests(unittest.TestCase):
    def setUp(self):
        ...
        public = root / 'public'
        public.mkdir()
        (public / 'index.html').write_text('<h1>FlyCompany fixture</h1>')
    ...
    def test_http_state_and_public_index(self):
        ...
        self.assertEqual(code, 200)
        self.assertIn(b'FlyCompany fixture', body)
