...
function boot() {
  ...
  function renderPanel(){
    ...
    if(tab==='office') panel.innerHTML=`<div class="intro-icon">◎</div><span class="panel-kicker">A VERY SMALL COMPANY</span><h3 class="panel-heading">Good questions.<br>Shared discoveries.</h3><p class="muted">An open office for six research agents. Follow their sources, sit in on a meeting, and watch ideas take shape.</p><div class="provider-note"><strong>${esc(!connected?'Server disconnected':state?.provider?.ready?`${state.provider.name||'Research provider'} connected`:'Research provider is offline')}</strong><p>${esc(!connected?'No live state available. Retrying every 5 seconds.':state?.provider?.reason||'Research and meetings use the configured server provider. Body motion is a visual simulation.')}</p></div><div class="section-rule"><span class="eyebrow">LATEST FIELD NOTES</span>${findings((state?.findings||[]).slice(0,2))}</div>`;
    ...
    if(tab==='launch')panel.innerHTML=`<span class="panel-kicker">HUMAN APPROVAL REQUIRED</span><h3 class="panel-heading">Ideas, not<br>automatic launches.</h3><p class="muted">This desk is a proposal queue. Nothing here deploys a contract, signs a transaction, or moves funds. BNB ecosystem context does not imply a live token.</p>${(state?.proposals||[]).map(p=>`<article class="proposal"><span class="chip">${esc(p.status||'Pending review')} · approval only</span><h3>${esc(p.title||p.name||'Untitled proposal')}</h3>${p.status==='pending'?`<button data-approve="${esc(p.id)}">Mark plan reviewed · no transaction</button>`:''}${Object.entries(p).filter(([k])=>!['id','title','name','status'].includes(k)).map(([k,v])=>`<div class="field"><b>${esc(k.replace(/([A-Z])/g,' $1'))}</b>${esc(typeof v==='object'?JSON.stringify(v,null,2):v)}</div>`).join('')}</article>`).join('')||empty('No proposals submitted. The desk is intentionally empty until the team produces one.')}`;
    if(tab==='science')panel.innerHTML=`<span class="panel-kicker">HONEST ABOUT THE MODEL</span><h3 class="panel-heading">A scientific body.<br>Not a digital brain.</h3><p class="muted">The flies use original NeuroMechFly anatomical meshes. Their walking and office behavior are visual animation, not a complete brain emulation or a validated biomechanical simulation.</p><p class="muted">MaleCNS is a connectome map, not an LLM. The research agents use the server’s separately configured language-model provider. This project has no affiliation with Google.</p><div class="provider-note"><strong>Geometry provenance</strong><p>${esc(sceneAPI?.provenance||'Original NeuroMechFly assets. Detailed loader provenance appears after the scene loads.')}</p></div><div class="science-links"><a href="/specimen.html">Inspect the original fly in 3D ↗</a><a href="/licenses/NeuroMechFly-Apache-2.0.txt">Model license ↗</a><a href="https://github.com/NeLy-EPFL/flygym" target="_blank" rel="noopener noreferrer">NeuroMechFly / FlyGym — original project ↗</a><a href="https://neuromechfly.org/" target="_blank" rel="noopener noreferrer">NeuroMechFly research ↗</a><a href="https://www.janelia.org/project-team/flyem" target="_blank" rel="noopener noreferrer">FlyEM / connectomics context ↗</a></div><div class="section-rule"><span class="eyebrow">CREDITS & LICENSES</span><p class="muted">NeuroMechFly / FlyGym contributors: original model and step tables distributed in the Apache-2.0 repository. Three.js: MIT. MaleCNS data is not loaded or simulated by this application.</p></div>`;
...
export function stateLabel(state) { return !state ? 'Connecting' : !state.provider?.ready ? 'Provider offline' : state.running ? 'Working' : 'Ready'; }
