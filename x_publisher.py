...
class XPublisher:
    ...
    def __init__(self, env=None):
        ...
        self.enabled = env.get('X_POST_MEETINGS', 'true').strip().lower() in {'1', 'true', 'yes', 'on'}
        self.consumer_key = env.get('X_CONSUMER_KEY', '').strip()
        self.consumer_secret = env.get('X_CONSUMER_SECRET', '').strip()
        self.bearer_token = env.get('X_BEARER_TOKEN', '').strip()
        self.access_token = env.get('X_ACCESS_TOKEN', '').strip()
        self.access_token_secret = env.get('X_ACCESS_TOKEN_SECRET', '').strip()
