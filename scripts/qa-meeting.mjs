...
const actual=await (await p.request.get('http://127.0.0.1:4775/api/state')).json();
...
await p.goto('http://127.0.0.1:4775');
...
await p.goto('http://127.0.0.1:4775');await p.locator('#loading').waitFor({state:'hidden',timeout:95000});
